# 07.容器网络-38-[service]_从外界连通Service与Service调试“三板斧”
...
而第三种方式，是 Kubernetes 在 1.7 之后支持的一个新特性，叫作 External Name。举个例子：
...
通过上述讲解不难看出，所谓 **Service，其实就是 Kubernetes 为 Pod 分配的、固定的、基于 iptables（或者 IPVS）的访问入口。**而这些**访问入口代理的 Pod 信息，则来自于 Etcd，由 kube-proxy 通过控制循环来维护**。
