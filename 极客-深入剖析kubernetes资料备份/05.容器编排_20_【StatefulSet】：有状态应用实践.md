# 05.容器编排-20-【StatefulSet】：有状态应用实践
...
![img](assets/bb2d7f03443392ca40ecde6b1a91c002.png)
...
> <font color="red">标注：更多介绍参见博客：[MySQL数据备份与恢复之Percona XtraBackup](https://blog.csdn.net/sinat_29214327/article/details/81517233)</font>
...
![img](assets/16aa2e42034830a0e64120ecc330a509.png)
...
```shell
 ...
 [[ `hostname` =~ -([0-9]+)$ ]] || exit 1
 ordinal=${BASH_REMATCH[1]}</div></td></tr><tr><td class="hljs-ln-numbers"><div class="hljs-ln-line hljs-ln-n" data-line-number="15"></div></td><td class="hljs-ln-code"><div class="hljs-ln-line"> [[ $ordinal -eq 0 ]] && exit 0
...
```shell
$ kubectl create -f mysql-statefulset.yaml</div></td></tr><tr><td class="hljs-ln-numbers"><div class="hljs-ln-line hljs-ln-n" data-line-number="2"></div></td><td class="hljs-ln-code"><div class="hljs-ln-line">$ kubectl get pod -l app=mysql
