# 05.容器编排-21-【DaemonSet】：容器化守护进程的意义
...
而在今天这篇文章中，我会为你重点讲解一个相对轻松的知识点：<font color="red">**DaemonSet**</font>。
...
1. 各种**网络**插件的 **Agent 组件**，都必须运行在每一个节点上，用来处理这个节点上的容器网络；
2. 各种**存储**插件的 **Agent 组件**，也必须运行在每一个节点上，用来在这个节点上挂载远程存储目录，操作容器的 Volume 目录；
